...
**Quality — paper validated:**
- 4-bit needle accuracy: **100% at all context lengths** — identical to baseline
- 4-bit perplexity degradation: **+2.2%** — essentially zero quality loss
- 3-bit needle accuracy: 100% at 1024–4096 context, 95% at 512 (short context noise)
- 3-bit perplexity degradation: +11.7% — noticeable but still usable

**Compression:**
- 4-bit: **4.0× exact** (0.136 GB vs ~0.53 GB fp16 KV cache at 4096 context) ✓
- 3-bit: **~2× actual** — the trivial uint8 packing (1 index/byte) limits real compression; true 5.3× would require packing 8 indices into 3 bytes

**Speed:**
- At 512–2048 context: TurboQuant is **3–4× slower** than baseline (9.5 vs 34 tok/s). Cause: 112 kernel launches per decode step (28 layers × 4 ops), each with a grid of N=8 — launch overhead dominates on a consumer GPU
- At 4096 context: TurboQuant 4b **beats baseline** (5.9 vs 4.7 tok/s) — baseline overflows VRAM and falls back to WDDM paging; compressed KV cache reduces decode-phase memory pressure
- The paper's speed results used A100/H100 where kernel launch overhead is amortized differently

**End VRAM measurement note:**
End VRAM appears slightly higher for TurboQuant than baseline — this is a measurement artifact. The baseline `DynamicCache` gets garbage-collected before measurement (end VRAM excludes the KV cache), while the `TurboQuantCache` object is held explicitly (end VRAM includes the compressed cache). The actual saving is visible in `avg_kv_cache_gb`: 0.136 GB (4b) vs ~0.53 GB (fp16).
